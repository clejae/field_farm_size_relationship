...
def plot_apu(preds, pred_cols, ref_cols, apu_plot_pth, qcut=None):
    """
    ...
    for i, pred_col in enumerate(pred_cols):
        ...
        legend_elements = [Patch(facecolor="blue", edgecolor=None, label="Mean bias"),
        ...
        ax.set_ylabel('Mean bias/Precision/RMSE/MAPE')
        ax.set_xlabel('Farm sizes [ha]')

        text = f"{n:,} fields\nMean bias: {round(a, 1)}\nPrecision: {round(p, 1)}\nRMSE: {round(u, 1)}\nMAPE: {round(mape, 1)}"
        ax.annotate(text=text, xy=(.2, .7), xycoords="axes fraction", zorder=1)
        ax.tick_params(axis='x', labelrotation=45)
...
def main():
    ...
    preds_test = pd.merge(rfr_pred_test[["field_id", "farm_size_r", "rfr_pred"]],
                     bay_pred_test[["field_id", "farm_id", "farm_size_ha", "preds"]], left_on="field_id", right_on="field_id", how="outer")
    ...
    # preds_test.to_csv(r"models\rfr_comp_best_bayes_model_replicated\rfr+bayes_test_predictions.csv", index=False)

    # preds_test = pd.read_csv(r"models\rfr_comp_best_bayes_model_replicated\rfr+bayes_test_predictions.csv")
    plot_apu(
        preds=preds_test,
    ...
    accuracy_assessment(
        ...
        pred_cols=["rfr_pred", "bayes_pred"],
        ref_cols=["farm_size_r", "farm_size_ha"],
        out_pth=r"models\rfr_comp_best_bayes_model_replicated_same_ids\scatter_plot_rfr+bayes_test.png")
